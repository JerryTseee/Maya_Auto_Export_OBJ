...
def process_multiple_animation_sequences(sequence_dir, frame_value_dir, export_dir):
    #get the list of animation sequence files in the selected directory
    sequence_files = os.listdir(sequence_dir)

    #get the list of frame value
    frame_value_files = os.listdir(frame_value_dir)

    #sort them
    sequence_files.sort()
    frame_value_files.sort()
    ...
    for sequence, frame in zip(sequence_files, frame_value_files):
        #construct the paths
        sequence_path = os.path.join(sequence_dir, sequence)
        frame_path = os.path.join(frame_value_dir, frame)

        #start to process
        if sequence.endswith(".json") and frame.endswith(".json"):
            #calling all function!!! Let's go!!!
            mgApplyFaceMocap(sequence_path)
            frame_number = get_frame_numbers(frame_path)
            ...
            print("Skip the mismatched file: "+sequence+" and "+frame)

sequence_dir = "F:\\Jerry\\animation_sequences"
frame_value_dir = "F:\\Jerry\\frame_values"
export_dir = "F:\\Jerry\\Automation_Multiple_Maya_OBJ"

#call the function
process_multiple_animation_sequences(sequence_dir, frame_value_dir, export_dir)
